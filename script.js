...
// --- 5-1. 料理を作るボタン（メインクリック）を押した時（ランダム料理＆金額完全同期ビルド） ---

// 🌟 あなたが用意してくれたランダム料理の画像リスト
...
if (mainClickBtn) {
    mainClickBtn.addEventListener('click', (e) => {
        ...
        const facilityBonus = totalFacilityCount * 0.1; // スタッフ1人につき +0.1円
        ...
        // 最終的な1クリックの稼ぎ（小数点以下は切り捨て）
        const clickProfit = Math.floor((baseClickPower + facilityBonus) * totalMultiplier);
        ...
        if (clickArea) {
            ...
            // ボタンの中の文字などを踏んでしまった場合のズレをミリ単位で補正する計算
            ...
            // 🌟【修正】邪魔だった頭の「🍳 」を取り除き、純粋な金額（+100 など）だけに直しました！
        ...
        // 1秒後の自動更新を待たずに、手動クリックの数字だけはその場ですぐ最新にする
        const mEl = document.getElementById('money');
...
// 【計算】本家と全く同じ平方根（ルート）を使った数式
// 🌟【本家仕様】チップ1枚目をもらうには、今世売上が最低「1兆（1 T）」必要です！
function calculateTotalChips(money) {
    if (money < 1e12) return 0; // 1兆（1,000,000,000,000）未満なら0個
    
    // 本家の公式数式： 割った値のルートをとり、小数点以下を切り捨てる
    return Math.floor(Math.sqrt(money / 1e12)); 
}

// 転生モーダルが開かれた瞬間に、表示を計算して書き換える処理
if (ascendBtn) {
    ascendBtn.addEventListener('click', () => {
        const totalEarnedChips = calculateTotalChips(gameState.seasonMoney);
        // 【本家仕様】これまでに獲得したチップを引いて、今回の純粋な「追加分」を出す
...
// 【ロード】ページを開いた時に、保存されたデータを自動で引き継群関数
function loadGame() {
    ...
    try {
        ...
        if (saveData.saveTime && gameState.mps > 0) {
            ...
            if (passedSeconds >= 60) {
                ...
                setTimeout(() => {
                    // ※もし formatSeconds 関数をまだ作っていない場合は、上の回答にあった定義を追加してくださいね
